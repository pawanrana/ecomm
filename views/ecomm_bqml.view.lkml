    ...
    sql: (SELECT * except(user_id1) FROM
              (SELECT user_id
                ...
                FROM (
                  ...
                  FROM `pawan-argolis-demo.ecomm.order_items` as oi
                  JOIN `pawan-argolis-demo.ecomm.inventory_items` as inv ON inv.id = oi.inventory_item_id
      ...
      JOIN (SELECT user_id as user_id1
            ...
            FROM `pawan-argolis-demo.ecomm.events`
...
view: ecomm_predict {
  derived_table: {
    sql: SELECT
      ...
      FROM
        ML.PREDICT(MODEL `pawan-argolis-demo.ecomm.kmeans_model5`,
        (SELECT * FROM
